...
function updateVulnStatus([string]$vulID,[string]$status){
 $Vulns=$stigText.CHECKLIST.STIGS.iSTIG.VULN 
  foreach($vuln in $Vulns){
   if($vuln.STIG_DATA.ATTRIBUTE_DATA -eq $vulID){
    $Vuln.STATUS=$status 
   }
  }
}
...
function updateStigCommentsField([string]$vulID,[string]$commentText){
 $comments=$stigText.CHECKLIST.STIGS.iSTIG.VULN  
  foreach($comment in $comments){
   if($comment.STIG_DATA.ATTRIBUTE_DATA -eq $vulID){
    $comment.FINDING_DETAILS=$commentText  
   }
  }
}
...
$status="Not_Reviewed"
try{
 $bootLoaderEntries=bcdedit | select-string "Windows Boot Loader"
...
function V_63365{
$hypervState=Get-WindowsOptionalFeature -FeatureName Microsoft-Hyper-V-All -Online | select State
$properties=net localgroup "Hyper-V Administrators"
&updateStigCommentsField "V-63365" `
("Hyper-V Status: $hypervState `n `n" + ($properties | Out-String))
}
   ...
   $cert=Get-ChildItem -Path Cert:Localmachine\disallowed -ErrorAction SilentlyContinue `
    ...
    if(($cert.Subject -eq "CN=DoD Root CA 2, OU=PKI, OU=DoD, O=U.S. Government, C=US" `
     -and $cert.Thumbprint -eq "22BBE981F0694D246CC1472ED2B021DC8540A22F" -and `
       ...
       ($cert.Subject -eq "CN=DoD Root CA 3, OU=PKI, OU=DoD, O=U.S. Government, C=US" `
     -and $cert.Thumbprint -eq "AC06108CA348CC03B53795C64BF84403C1DBD341" -and ` 
...
if($isClassified -eq "N"){
  try{
   $cert=Get-ChildItem -Path Cert:Localmachine\disallowed -ErrorAction SilentlyContinue `
    ...
    if(($cert.Subject -eq "CN=DoD Root CA 3, OU=PKI, OU=DoD, O=U.S. Government, C=US" `
...
try{
 if($OSReleaseID -ge 1709 -and $isClassified -eq "N"){
  ...
  if($processMitigation.DEP.Enable -ieq "ON" -and `
     $processMitigation.ASLR.ForceRelocateImages -ieq "ON" -and `   
     $processMitigation.ASLR.BottomUp -ieq "ON" -and `     
     $processMitigation.Payload.EnableExportAddressFilter -ieq "ON" -and `
     $processMitigation.Payload.EnableExportAddressFilterPlus -ieq "ON" -and `
     $processMitigation.Payload.EnableImportAddressFilter -ieq "ON" -and `
     $processMitigation.Payload.EnableRopStackPivot -ieq "ON" -and `
     $processMitigation.Payload.EnableRopCallerCheck -ieq "ON" -and `
     $processMitigation.Payload.EnableRopSimExec -ieq "ON"){
   $status = "NotAFinding"
   } else {$status = "Open"}
  } else {$status="Not_Applicable"}
} catch {
...
try{
 if($OSReleaseID -ge 1709 -and $isClassified -eq "N"){
  ...
  if($processMitigation.DEP.Enable -ieq "ON"){
   $status = "NotAFinding"
   } else {$status = "Open"}
  } else {$status="Not_Applicable"}
} catch {
...
try{
 if($OSReleaseID -ge 1709 -and $isClassified -eq "N"){
  ...
  if($processMitigation.DEP.Enable -ieq "ON" -and `
     $processMitigation.ASLR.ForceRelocateImages -ieq "ON" -and `   
     $processMitigation.ASLR.BottomUp -ieq "ON"){
   $status = "NotAFinding"
   } else {$status = "Open"}
  } else {$status="Not_Applicable"}
} catch {
...
try{
 if($OSReleaseID -ge 1709 -and $isClassified -eq "N"){
  ...
  if($processMitigation.DEP.Enable -ieq "ON" -and `        
     $processMitigation.ImageLoad.BlockRemoteImageLoads -ieq "ON" -and ` 
     $processMitigation.ChildProcess.DisallowChildProcessCreation -ieq "ON" -and ` 
     $processMitigation.Payload.EnableExportAddressFilter -ieq "ON" -and `
     $processMitigation.Payload.EnableExportAddressFilterPlus -ieq "ON" -and `
     $processMitigation.Payload.EnableImportAddressFilter -ieq "ON" -and `
     $processMitigation.Payload.EnableRopStackPivot -ieq "ON" -and `
...
function V_88203{
$status="Not_Reviewed"
$location="HKLM:\SOFTWARE\Policies\Microsoft\OneDrive\AllowTenantList\"
 
 $items = Test-Path $location 
  if($items) {
    $status = "Not_Reviewed"
    &updateStigCommentsField "V-88203" `
     ("Registry path exists.  Please manually confirm that the Key is set to the correct value.")    
  } else {
    $status = "Open"
  }
 &updateVulnStatus "V-88203" $status 
} 

function V_94719{
$status="Not_Reviewed"
$location="HKLM:\SOFTWARE\Policies\Microsoft\Windows\AppPrivacy\"
$key="LetAppsActivateWithVoice"

 if($OSReleaseID -ge 1903){
   $items = get-itemproperty $location -ErrorAction SilentlyContinue | select $key
   if ( $items.$key.count -eq 1 -and $items.$key -eq 2) {
    $status="Not_Applicable"
   } else {
     $key="LetAppsActivateWithVoiceAboveLock"
     $items = get-itemproperty $location -ErrorAction SilentlyContinue | select $key
     if ( $items.$key.count -eq 1 -and $items.$key -eq 2) {
       $status="NotAFinding"
     } else {
...
&updateVulnStatus "V-94719" $status
}
...
function main{
 ...
 cls
 Write-Output "Starting Checks of Not Reviewed STIG Items:"
